...
function sortRows(rows, columns, sortState) {
  ...
  const sorted = [...rows].sort((a, b) => {
    const first = String(getValue(a, column) ?? '').toLowerCase()
    const second = String(getValue(b, column) ?? '').toLowerCase()
...
function LoadingRows({ columns, density, selectable }) {
  return Array.from({ length: 5 }).map((_, rowIndex) => (
    <tr className="cl-data-table__row" key={`loading-${rowIndex}`}>
      ...
      {columns.map((column) => (
        <td className={cx('cl-data-table__cell', `cl-data-table__cell--${density}`)} key={column.key}>
...
export function DataTable({
  ...
  const processedRows = useMemo(() => sortRows(filterRows(data, columns, query), columns, sortState), [columns, data, query, sortState])
  ...
  const visibleKeys = visibleRows.map((row) => String(row[rowKey]))
      ...
      <div className={cx('cl-data-table__viewport', `cl-data-table__viewport--radius-${radius}`)}>
        <table className="cl-data-table__table">
          <thead className="cl-data-table__head">
            <tr>
              {selectable ? <th className="cl-data-table__header-cell cl-data-table__selection-cell" scope="col"><input aria-label="Select visible rows" checked={allVisibleSelected} className="cl-data-table__checkbox" onChange={toggleVisibleRows} type="checkbox" /></th> : null}
              {columns.map((column) => {
                const direction = sortState.key === column.key ? sortState.direction : undefined
                const isSortable = sortable && column.sortable !== false
                return (
                  <th className={cx('cl-data-table__header-cell', column.align && `cl-data-table__header-cell--align-${column.align}`)} key={column.key} scope="col">
          ...
          <tbody>
            ...
            {!loading && visibleRows.map((row) => {
              ...
              return (
                <tr className={cx('cl-data-table__row', isSelected && 'cl-data-table__row--selected')} key={key}>
                  {selectable ? <td className={cx('cl-data-table__cell cl-data-table__selection-cell', `cl-data-table__cell--${density}`)}><input aria-label={`Select row ${key}`} checked={isSelected} className="cl-data-table__checkbox" onChange={() => toggleRow(key)} type="checkbox" /></td> : null}
                  {columns.map((column) => {
                    const value = getValue(row, column)
                    return <td className={cx('cl-data-table__cell', `cl-data-table__cell--${density}`, column.align && `cl-data-table__cell--align-${column.align}`)} key={`${key}-${column.key}`}>{column.cell ? column.cell(row, value) : value}</td>
                  })}
                </tr>
              )
            })}
            {!loading && visibleRows.length === 0 ? <tr><td colSpan={columns.length + (selectable ? 1 : 0)}><div className="cl-data-table__empty"><div><p className="cl-data-table__empty-title">{emptyTitle}</p><p className="cl-data-table__empty-description">{emptyDescription}</p></div></div></td></tr> : null}
          </tbody>
        </table>
      </div>
